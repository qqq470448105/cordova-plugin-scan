...
-(void)setupTipTitleView{
    ...
    UIView*mask=[[UIView alloc]initWithFrame:CGRectMake(0, _maskView.sd_y+_maskView.sd_height, self.view.sd_width, 2*kBorderW)];
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        NSString *metadata = metadataObject.stringValue;
        if ([self.delegate respondsToSelector:@selector(scanSuccess:)])
        {
            [self.delegate scanSuccess:metadata];

        }
        
    }else
    {
        if ([self.delegate respondsToSelector:@selector(scanfalse:)])
        {
            [self.delegate scanfalse:@"没有扫描到结果"];
...
-(void)decodeImage:(UIImage*)image{
    ...
    if(iOS8){
        ...
        if (features.count >=1) {
            ...
            if ([self.delegate respondsToSelector:@selector(scanSuccess:)])
            {
                [self.delegate scanSuccess:scannedResult];   
            }
        }
        else{
            ...
            if ([self.delegate respondsToSelector:@selector(scanfalse:)])
            {
                [self.delegate scanfalse:@"该图片没有包含一个二维码！"];
            }
            ...
            if ([self.delegate respondsToSelector:@selector(scanfalse:)])
            {
                [self.delegate scanfalse:@"暂不支持iOS8以下版本扫码"];
