package cordova.plugin.scan.erweima.sweepcode.activity;
...
import com.ionicframework.transparentnavbar345049.R;


import java.io.IOException;
import java.util.concurrent.atomic.AtomicInteger;

import cordova.plugin.scan.erweima.sweepcode.handler.CaptureActivityHandler;
import cordova.plugin.scan.erweima.sweepcode.handler.InactivityTimer;
import cordova.plugin.scan.erweima.sweepcode.listener.OnRxScanerListener;
import cordova.plugin.scan.erweima.sweepcode.manager.CameraManager;
import cordova.plugin.scan.erweima.sweepcode.tools.AnimationTool;
import cordova.plugin.scan.erweima.sweepcode.tools.BeepTool;
import cordova.plugin.scan.erweima.sweepcode.tools.PhotoTool;
import cordova.plugin.scan.erweima.sweepcode.tools.QrBarTool;
...
//        RxBarTool.setNoTitle(this);
    setContentView(R.layout.activity_scaer_code_layout);
...
//      finish();
      ...
      mIntent.putExtra("resultString", "");
      // 设置结果，并进行传送
      setResult(2, mIntent);
      finish();
    } else if (viewId == R.id.top_openpicture) {
      PhotoTool.openLocalImage(mContext);
    }
  }
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (resultCode == RESULT_OK) {
      ...
      try {
        ...
        if (rawResult != null) {
          Intent mIntent = new Intent();
          mIntent.putExtra("resultString", rawResult.getText());
          // 设置结果，并进行传送
          setResult(2, mIntent);
          finish();
...
//          }
  ...
  private void initDialogResult(Result result) {
    ...
    runOnUiThread(new Runnable() {
      @Override
      public void run() {
        Toast.makeText(ActivityScanerCode.this, realContent + "", Toast.LENGTH_SHORT).show();
      }
    });
...
//        RxSPTool.putContent(mContext, RxConstants.SP_SCAN_CODE, RxDataTool.stringToInt(RxSPTool.getContent(mContext, RxConstants.SP_SCAN_CODE)) + 1 + "");
  ...
  public void handleDecode(Result result) {
    ...
    BeepTool.playBeep(mContext, vibrate);//扫描成功之后的振动与声音提示

    String result1 = result.getText();
    Log.v("二维码/条形码 扫描结果", result1);
    if (!TextUtils.isEmpty(result1.toString().trim())) {
      Intent mIntent = new Intent();
      mIntent.putExtra("resultString", result1);
      // 设置结果，并进行传送
      setResult(2, mIntent);
      finish();
    }
